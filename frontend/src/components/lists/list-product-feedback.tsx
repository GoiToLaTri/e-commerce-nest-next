...
export default function ListProductFeedback({
  ...
  const { data, isLoading, isFetching } = useGetProductReviews(productId);
  // console.log(data);
  return (
    <div className="bg-[#241932] border-[2px] border-solid border-[#302540] rounded-2xl p-6 flex flex-col items-start gap-4 shadow-lg sm:col-span-2 w-[60%]">
      ...
      <div className="max-h-[40rem] h-full w-full">
        <div className="bg-[#352C43] rounded-2xl w-full overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-none [&::-webkit-scrollbar-thumb]:bg-slate-200 [&::-webkit-scrollbar-thumb]:rounded-xl">
          {(isLoading || isFetching) && <LoadingSpin />}
          {!isLoading &&
            !isFetching &&
          ...
          {data &&
            data.map((item) => <FeedbackCard key={item.id} data={item} />)}
        </div>
      </div>
    </div>
  );
